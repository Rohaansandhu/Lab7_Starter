describe('Basic user flow for Website', () => {
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    await page.evaluate(() => {
      ...
      for (let i = 1; i < productItems.length; i++) {
        productItems[i].shadowRoot.querySelector('button').click();
      }
    });
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const allRemove = await page.evaluate(() => {
      ...
      for (let i = 0; i < productItems.length; i++) {
        if (productItems[i].shadowRoot.querySelector('button').innerText !== 'Remove from Cart') {
          return false;
        }
      }
      return true;
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const allAdd = await page.evaluate(() => {
      const productItems = document.querySelectorAll('product-item');
      for (let i = 0; i < productItems.length; i++) {
        if (productItems[i].shadowRoot.querySelector('button').innerText !== 'Add to Cart') {
          return false;
        }
      }
      return true;
